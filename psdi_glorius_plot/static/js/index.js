...
function fillExample() {
  ...
  updateROCrateForm(false, true);

  // Fill in the standard conditions info
  updateQuillContents("#rocrate-baseline-desc", "To a 10 mL Schlenk tube equipped with a Teflon-coated magnetic stir " +
    "bar, BCB (0.2 mmol, 1.0 equiv.), Na<sub>2</sub>CO<sub>3</sub> (2.0 equiv.) and <em>fac</em>-Ir(ppy)3 " +
    "(0.03 equiv.) were added. The Schlenk tube was evacuated and backfilled with argon three times. Subsequently, " +
    "<em>tert</em>-butyl bromoacetate (0.22 mmol, 1.1 equiv.) and MeCN:CH<sub>2</sub>Cl<sub>2</sub> (1:1) 1.0 mL " +
    "were added against the positive flow of argon. For entry 5, the reaction was prepared similar under air.  All " +
    "reactions were then stirred under irradiation with blue LEDs (30 W, λ<sub>max</sub> = 450 nm) at rt (30 °C to " +
    "35 °C) for 2 h. Later, the reaction mixture passed through a celite plug and the solvent was removed in vacuo. " +
    "The yield was determined by crude NMR.");
  ...
  // Select the license
  $("#rocrate-license-cc-by-4\\.0").prop("checked", true).change();
  ...
  $("#rocrate-email-input").val("glorius@uni-muenster.de");

  // Set the citation on a delay, since it depends on the plot title which will take a moment to update
  userHasEditedCitation = true;
  setTimeout(() => {
    let datasetTitle = cleanTags(getQuillEditorHTML("#rocrate-title-input"));
    const date = (new Date()).toISOString().slice(0, 10);
    updateQuillContents("#rocrate-citation", "Please cite: <em>F.Zhang, S.Dutta, A.Petti, D.Rana, C. G.Daniliuc, " +
      "F.Glorius</em>, Angew. Chem. Int. Ed.2025, 64, e202418239. https://doi.org/10.1002/anie.202418239. " +
      `${datasetTitle} generated by PSDI Glorius Plot Generator version ${version} (${date}).`);
  }, 100)
...
async function updateROCrateForm(firstTime = false, example = false) {
  ...
  if (firstTime || example) {
    initCondDescs();
    enableQuillEventsAndCallbacks();
    setTimeout(useDefaultROCrateTitleDesc, 100);
  }
  if (firstTime && !userHasEditedCitation) {
